...
SELECT S.STUD_CODE,S.STUD_NAME,B.BOOK_CODE,BB.BOOK_NAME 
FROM STUDENT_MASTER S,BOOK_TRANSACTION B, BOOK_MASTER BB 
WHERE S.STUD_CODE=B.STUD_CODE
AND TO_CHAR(B.BOOK_EX_RETURN_DATE,'DD MM YYYY')=TO_CHAR(SYSDATE,'DD MM YYYY') AND BB.BOOK_CODE=B.BOOK_CODE;

--------------------------------------------------------------------------------------------------------------------------------
/*3.1 4*/

SELECT STAFF_MASTER.STAFF_CODE, STAFF_NAME, DESIGN_NAME, DEPT_NAME,
BOOK_TRANSACTION.BOOK_CODE, BOOK_NAME, BOOK_ISSUE_DATE
FROM STAFF_MASTER,DEPARTMENT_MASTER,DESIGNATION_MASTER,BOOK_MASTER,BOOK_TRANSACTION
WHERE TO_CHAR(BOOK_ISSUE_DATE) BETWEEN TO_CHAR(SYSDATE) AND TO_CHAR(SYSDATE-30) 
AND STAFF_MASTER.STAFF_CODE=BOOK_TRANSACTION.STAFF_CODE
AND DESIGNATION_MASTER.DESIGN_CODE=STAFF_MASTER.DESIGN_CODE 
AND DEPARTMENT_MASTER.DEPT_CODE=STAFF_MASTER.DEPT_CODE
AND BOOK_TRANSACTION.BOOK_CODE=BOOK_MASTER.BOOK_CODE;

---------------------------------------------------------------------------------------------------------------
/*3.1 6*/
SELECT BOOK_PUB_AUTHOR,BOOK_NAME,COUNT(BOOK_PUB_AUTHOR) FROM BOOK_MASTER GROUP BY BOOK_NAME HAVING COUNT(BOOK_PUB_AUTHOR)>1;

------------------------------------------------------------------------------------------------------------------------------------
/*3.1 7*/
SELECT BOOK_,BOOK_NAME FROM BOOK_MASTER GROUP BY AUTHORNAME HAVING COUNT(AUTHOR)>1;
